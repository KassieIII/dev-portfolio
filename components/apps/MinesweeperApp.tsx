...
export default function MinesweeperApp() {
  ...
  return <div className="mines-app"><header><div><strong>{lost ? "Mine exploded" : won ? "Field cleared!" : "KY Mines"}</strong><span><Flag size={13} /> {config.mines - flags}<Timer size={13} /> {String(seconds).padStart(3,"0")}</span></div><div className="mine-modes">{(Object.keys(modes) as Mode[]).map((item) => <button className={mode === item ? "active" : ""} onClick={() => restart(item)} key={item}>{item}</button>)}</div><button className="mine-reset" onClick={() => restart()} aria-label="Restart"><RotateCcw size={16} /></button></header><div className="mine-board" style={{ gridTemplateColumns: `repeat(${config.cols}, 1fr)` }}>{cells.map((cell,index) => <button key={index} onClick={() => reveal(index)} onContextMenu={(event) => flag(event,index)} className={`${cell.revealed ? "revealed" : ""} n${cell.adjacent} ${cell.mine ? "mine" : ""}`} aria-label={`Cell ${index + 1}`}>{cell.flagged && !cell.revealed ? "⚑" : cell.revealed && cell.mine ? "✹" : cell.revealed && cell.adjacent ? cell.adjacent : ""}</button>)}</div><footer>Click to reveal · right-click to flag · first move is always safe</footer></div>;
}
