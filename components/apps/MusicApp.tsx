...
export function MusicWidget({ onOpen }: { onOpen: () => void }) {
  ...
  return <aside className="music-widget widget" aria-label="Now playing"><button className="music-widget-main" onClick={onOpen}>{current?.artworkUrl100 ? <img src={current.artworkUrl100.replace("100x100", "300x300")} alt="" /> : <span className="music-widget-art">KY</span>}<span><small>{playing ? "NOW PLAYING" : "KY MIX READY"}</small><strong>{current?.trackName ?? (loading ? "Loading global mix…" : "KY Mix")}</strong><em>{current?.artistName ?? "Global city-pop mix"}</em></span></button><div><button onClick={() => step(-1, true)} aria-label="Previous"><SkipBack size={15} /></button><button onClick={toggle} aria-label={playing ? "Pause" : "Play"}>{playing ? <Pause size={16} /> : <Play size={16} />}</button><button onClick={() => step(1, true)} aria-label="Next"><SkipForward size={15} /></button></div></aside>;
}
